...
Single-player real-time strategy with zero humans on the battlefield: you command one AI overlord (a CENTCOM base) against another, mining resources and building drones from real-world specs. No multiplayer yet: the WebSocket relay exists (C-06) but nothing pairs two clients into a match. Think Warcraft 2 / StarCraft economy loop plus Command & Conquer satellite recon, but every unit is a real drone modeled 1:1: real weight, battery capacity, wind limits, ballistics, collision behavior.
