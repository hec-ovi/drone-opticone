...
Real-time strategy with zero humans on the battlefield, playable in the browser right now: you command an AI overlord (a CENTCOM base) against another, mining resources and building drones from real-world specs, from the start menu to the victory screen. Today it is you versus the AI; multiplayer may land later if the project grows and people ask for it (the WebSocket transport is already built and tested). Think Warcraft 2 / StarCraft economy loop plus Command & Conquer satellite recon, but every unit is a real drone modeled 1:1: real weight, battery capacity, wind limits, ballistics, collision behavior.
...
- **Economy.** Lithium for batteries and power, oil cracked into plastics, credits from ore deposits and market trade, a power grid over it all. Fog of war, satellite passes for visibility.
